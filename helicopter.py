...
"""
    ...
    def __init__(self, env_mode = 'stochastic',
                 ...
                 ip_tree = None, ip_empty = None, ip_fire = None, ip_rock = None, ip_lake = None,pos_row=None,pos_col=None):
    ...
    #Features Added by Mau
    def Copia(self):
        # Does a simple copy to use for sample recollection in parallel of the object             
        NEW = EnvMakerForestFire(init_pos_row=self.pos_row,init_pos_col=self.pos_col,n_row = self.n_row, n_col = self.n_col,                                      p_tree = self.p_tree, p_fire =self.p_fire,
                 moves_before_updating = self.moves_before_updating,reward_type = self.reward_type, reward_tree = self.reward_tree,                          reward_fire = self.reward_fire, reward_empty =self.reward_empty, reward_hit = self.reward_hit,                 
                 sub_tree = self.sub_tree, sub_empty = self.sub_empty, sub_fire = self.sub_fire, sub_rock = self.sub_rock, sub_lake =                        self.sub_lake,ip_tree = self.ip_tree, ip_empty =self.ip_empty, ip_fire =self.ip_fire, ip_rock = self.ip_rock,
                 ip_lake = self.ip_lake)
        NEW.grid = self.grid      
    ...
    def make_checkpoint(self): 
        ...
        self.checkpoints.append((self.grid, self.pos_row, self.pos_col, self.total_hits, self.total_reward, self.remaining_moves))
    ...
    def load_checkpoint(self, checkpoint_id):
        ...
        try:
            self.grid, self.pos_row, self.pos_col, self.total_hits, self.total_reward ,self.remaining_moves= self.checkpoints[checkpoint_id]
        except:
