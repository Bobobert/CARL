...
def get_action_heuristic(grid, position, env, greedy=False):
    ...
    return action
"""
def get_action_greedy(grid, position, env):
    fire_actions, tree_actions, empty_actions = neighborhood_cells_to_actions(grid, position, env)
    if fire_actions:
"""
...
if __name__ == '__main__':
    # Environment
    env_ = helicopter.Helicopter(n_row = N_ROW, n_col = N_COL,
                            p_fire = P_FIRE, p_tree = P_TREE,
                            tree = TREE, fire = FIRE, empty = EMPTY, freeze=1)
    env = env_.Copy()
    ...
    ckp = 0
    total_reward = 0
    for i in range(env.freeze * 30):
        action = get_action_heuristic(grid, position, env, False)
